...
public class RestServlet extends HttpServlet implements ResourceManager {
	...
	public void service(HttpServletRequest rq, HttpServletResponse rsp) throws IOException {
		...
		System.out.println(rq.getRemoteAddr() + ":" + pathInfo);
	...
	public void setUser(HttpServletRequest rq, User user) throws Exception {
		if (user == null)
			rq.getSession().removeAttribute("user");
		else {
			String s = codec.enc().put(user).toString();
			rq.getSession().setAttribute("user", s);
		}
	}

	public User getUser(HttpServletRequest rq) {
		try {
			Object s = rq.getSession().getAttribute("user");
			if (s == null)
				return null;

			return codec.dec().from((String) s).get(User.class);
		}
		catch (Exception e) {
			return null;
		}
